...
def _process_video_scene_modality(frames: list, fps, example: str, offset: float, duration: float, modality) -> dict:
    ...
    if extracted_frames_count < modality.config.EXTRACTED_FRAMES_COUNT:
    ...
    if len(result_list_resized) != modality.config.EXTRACTED_FRAMES_COUNT:
        delta = modality.config.EXTRACTED_FRAMES_COUNT - len(result_list_resized)
    ...
    with TfExampleWriter(name, tf_dataset_path, samples_per_tfrecord) as writer:
        ...
        for file_name, annotations_list in config.ANNOTATIONS:
            ...
            for emotion in annotations_list:
                ...
                frame_duration: float = min(TimeDependentModality.WINDOW_WIDTH_IN_SECS, emotion.duration)

                while offset + frame_duration <= emotion.offset + emotion.duration:
                    features_by_name = {}
